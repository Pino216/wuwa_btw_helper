...
#### 蒙特卡洛算法性能优化
在v1.5版本中，我们引入了**蒙特卡洛树搜索（MCTS）** 和**快速蒙特卡洛**算法，并针对性能进行了专门优化：

1. **异步分片执行**：批量模拟时采用异步分片技术，避免阻塞用户界面
2. **智能次数控制**：根据算法复杂度动态调整模拟次数（蒙特卡洛50-100次，其他算法1000次）
3. **稳定性模式**：蒙特卡洛算法支持稳定性模式，通过多次运行取平均值减少随机波动
4. **进度实时反馈**：在批量测试过程中实时显示每个算法的测试进度
...
- **蒙特卡洛算法**在复杂局面下表现优异，但计算成本较高，适合中后期使用
- 通过异步分片技术，可以在不卡死界面的情况下完成蒙特卡洛批量测试
