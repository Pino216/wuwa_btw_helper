...
- **流程记录与导出**：手动模拟时自动记录每一步操作的完整状态，支持导出为JSON文件
- **撤销操作**：支持撤销上一步操作，回退到操作前的状态（包括网格、点击次数、未触发计数等）
...
### v1.4 (当前版本)
- **标题样式优化**：移除文字渐变蒙版，采用纯白色文字，修复 emoji 色彩丢失问题
- **撤销功能修复**：修正撤销操作无法正确恢复状态的问题，在撤销后保存状态至 localStorage
- **步数显示功能**：新增在矩阵上显示每个已开格子首次打开步数的能力，默认显示勾号✅；可在“显示步数”复选框中切换为数字步数显示，步数不会被后续操作覆盖
- **折叠模块修复**：修复左侧参数栏中“概率权重”“保底系统”“批量模拟”等模块无法正常展开/折叠的问题；改用内联事件绑定，彻底解决 DOM 操作后事件失效的 bug
- **其他改进**：优化了核心算法与数据处理的稳定性
- **修复错字**：修正文档中的错别字

### v1.3
- **流程记录与导出**：新增操作日志记录功能，可导出为JSON文件（包含每一步的网格、概率、保底等完整状态）
- **撤销操作**：新增撤销上一步功能，可回退到上一步操作前的状态
- **修复撤销功能无法正常工作的bug**
